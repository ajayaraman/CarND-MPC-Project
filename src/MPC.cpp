 ...
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    ...
    for (int t = 1; t < N; t++) {
      ...
      //Calculate trajectory with kinematics model
...
vector<double> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs) {
  ...
  size_t n_vars = numStates * N + numActuators * (N-1);
  ...
  //We return the actuator values in the first two elements. 
  vector<double> result;
  result.push_back(solution.x[delta_start]);
  result.push_back(solution.x[a_start]);

  //Add trajectory information to plot mpc trajectory solved by the optimizer for debuggin
  for(i = 0; i < N-1; i++)
  {
